...
>
> Ultima sessione (2026-06-12): creata pagina ruling YGO + bottone sulla pagina carta. Nuovo file `app/cards/[game]/[slug]/ruling/page.tsx` (server component, pubblico, nessun `getUser()`): per `game !== 'yugioh'` → `notFound()`; fetcha `select('name, image_url, ruling_data, konami_id')` da `cards` su `game='yugioh'` e `slug`; se carta non trovata o nessuna sezione di `ruling_data` ha almeno un'entry con `en` → card neobrutalist "Nessun ruling disponibile" con bottone "← Torna alla carta". Altrimenti renderizza ogni `effectKey` di `ruling_data` (tipo `RulingData = Record<string, Partial<Record<'en'|'it'|'de'|'fr'|'es'|'pt', string>>[]>`, ordinate numericamente via `parseFloat`) in una card con bordo 2px/ombra 4px, titolo "Effetto {key}" (o "Regola generale" per `'0.5'`), elenco delle sole entry con `en` (altre lingue come `ja` ignorate). Breadcrumb Carte → Yu-Gi-Oh! → {nome carta} → Ruling, `generateMetadata` → `"{nome carta} Ruling | Huntlist"`, nota finale "Fonte: YGOResources" linkata a `https://db.ygoresources.com/card#{konami_id}` (`target="_blank" rel="noopener noreferrer"`). In `app/cards/[game]/[slug]/page.tsx`: aggiunti tipi `RulingEntry`/`RulingData` e campo `ruling_data: RulingData | null` a `Card` (già incluso da `select('*')`, nessuna modifica alla select necessaria); aggiunto bottone "Ruling" (icona `BookOpen` da lucide-react, `variant="outline"`, stile neobrutalist con press-effect già fornito da `Button`) sopra il blocco Descrizione, visibile solo per `game === 'yugioh' && card.ruling_data` e linkato a `/cards/yugioh/{slug}/ruling`. Non toccato `CardClient.tsx` — coerente con la deviazione architetturale già documentata nella sessione precedente (quel componente resta solo per il view-count, la UI di dettaglio carta vive in `page.tsx`). `npm run typecheck` e `npm run lint` puliti. Testato in preview (richiesto restart del dev server perché Turbopack non rilevava la nuova route annidata sotto `[slug]/ruling` a caldo): `/cards/yugioh/sky-striker-ace-kagari` mostra il bottone "Ruling" sopra l'effetto (ora in italiano, lo script rulings è stato eseguito nel frattempo); `/cards/yugioh/sky-striker-ace-kagari/ruling` → 200, sezioni "Regola generale"/"Effetto 1"/"Effetto 2" in EN + link YGOResources; `/cards/yugioh/abyss-flower/ruling` → 200 "Nessun ruling disponibile"; `/cards/pokemon/pikachu/ruling` → 404. Sessione precedente (effect_data su `import-yugioh-rulings.ts` e `page.tsx`, deviazione su `CardClient.tsx`), e sessioni ancora precedenti (concorrenza import rulings, creazione import-yugioh-rulings.ts, konami_id su import-yugioh.ts, fix build sitemap Magic, filtri Magic card_type, fix lint, rename `/carte`→`/cards` e `/feed`→`/market`) omesse per brevità — vedi git log per i dettagli. **Nota debito tecnico**: `konami_id`, `ruling_data` ed `effect_data` sono colonne aggiunte a mano su Supabase, non presenti in `supabase/migrations/0001_init.sql` — valutare una migrazione di follow-up.
