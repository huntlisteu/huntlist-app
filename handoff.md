...
>
> **Ultima sessione (2026-06-13, notte 7): bottone "Condividi" sulla pagina Hunt.**
> - **File creato**: `components/hunts/ShareHuntButton.tsx` — client component (`"use client"`). Bottone `Button` (`variant="outline"`, `size="sm"`, già neobrutalist: border-2 Ink/Char, shadow offset 4px, hover press effect) con icona `Share2` (lucide-react) e testo "Condividi". Al click, `navigator.clipboard.writeText(window.location.href)`; se va a buon fine, mostra icona `Check` + "Copiato!" per 2s (`FEEDBACK_DURATION_MS`, gestito con `useEffect`/`setTimeout` con cleanup) poi torna allo stato iniziale. Se `navigator.clipboard` non è disponibile (contesto non sicuro) → no-op silenzioso; se `writeText` rigetta (permesso negato) → catch silenzioso, nessun feedback.
> - **`app/(app)/hunts/[id]/page.tsx`**: import di `ShareHuntButton`; l'header della Hunt ora avvolge `<h1>{hunt.title}</h1>` e `<ShareHuntButton />` in un `div` `flex flex-wrap items-start justify-between gap-3`, così il bottone è visibile accanto al titolo su desktop e va a capo sotto su schermi stretti.
> - **Verifica**: `npm run typecheck` e `npm run lint` puliti. Creata temporaneamente `app/dev-preview-share/page.tsx` (titolo + bottone, senza dati Hunt reali) per bypassare l'assenza di Hunt `open` nel DB; in preview (Chromium headless) `navigator.clipboard.writeText` rigetta con "Document is not focused" — comportamento noto dei browser headless, gestito correttamente dal catch silenzioso del componente. Stubbando `navigator.clipboard.writeText` per simulare il permesso concesso: click → bottone passa a icona `Check` + "Copiato!", dopo 2s torna a `Share2` + "Condividi". Classi renderizzate confermano border-2/shadow offset/hover-press e varianti `dark:` corrette. Pagina di preview rimossa a fine sessione.
>
> **Sessione precedente (2026-06-13, notte 6): rinomina Feed→Market e riordino bottom nav mobile.**
