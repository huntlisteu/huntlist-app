...
>
> Ultima sessione (2026-06-12): aggiunta estrazione `effect_data` (testo effetto YGOResources EN/IT/DE/FR/ES/PT) alla nuova colonna `cards.effect_data jsonb`. In `scripts/import-yugioh-rulings.ts`: nuovo tipo `EffectData = Partial<Record<RulingLang, string>>`, `YGOResourcesResponse` estesa con `cardData?: Partial<Record<AllLang, { effectText?: string }>>`, nuova funzione `extractEffectData()` (stesso filtro lingue di `extractRulingData`, scarta ja/ko/cn/ae), `processCard()` ora salva sia `ruling_data` che `effect_data` nello stesso update e ritorna anche `hasEffect`; log con contatore `withEffects`. Script **ancora non eseguito** (da lanciare manualmente — Simone). In `app/cards/[game]/[slug]/page.tsx`: aggiunto tipo `EffectData` e campo `effect_data: EffectData | null` a `Card`; per `game === 'yugioh'` calcolato `displayDescription = card.effect_data?.it ?? card.effect_data?.en ?? card.description` (altri giochi invariati), usato sia nel blocco "Descrizione" (con `whitespace-pre-line` perché i testi YGOResources contengono `\n`) sia in `jsonLd.description`. `select('*')` in `getCard()` include già `effect_data`, nessuna modifica necessaria alla select. **Deviazione dal task**: non ho toccato `CardClient.tsx` come richiesto letteralmente — quel componente è `'use client'` solo per l'incremento view-count via RPC e non renderizza alcuna descrizione; la logica di fallback è stata implementata in `page.tsx` (server component), dove `card.description` era già renderizzato — architetturalmente più corretto (niente client component superfluo, SEO server-rendered). `npm run typecheck` e `npm run lint` puliti su entrambi i file. Testato in preview: `/cards/yugioh/sky-striker-ace-kagari` renderizza correttamente con fallback a `card.description` (effect_data ancora `null`, script non eseguito). **Nota debito tecnico**: `konami_id`, `ruling_data` ed `effect_data` sono colonne aggiunte a mano su Supabase, non presenti in `supabase/migrations/0001_init.sql` — valutare una migrazione di follow-up. Sessioni precedenti (concorrenza import rulings, creazione import-yugioh-rulings.ts, konami_id su import-yugioh.ts, fix build sitemap Magic, filtri Magic card_type, fix lint, rename `/carte`→`/cards` e `/feed`→`/market`) omesse per brevità — vedi git log per i dettagli.
