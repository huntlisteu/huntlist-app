...
>
> Ultima sessione (2026-06-12): fix build failure — `npm run build` falliva perché Next.js tentava di prerenderare staticamente tutti i chunk delle sitemap per-gioco a build time, e il chunk Magic (righe 145000-145999) andava in `statement timeout` su Supabase. Sostituito `export const revalidate = 86400` con `export const dynamic = 'force-dynamic'` nei 4 file `app/cards/s/{yugioh,pokemon,one_piece,magic}/sitemap.ts` (logica identica via `lib/cardSitemap.ts`, condivisa tra i 4) — ora generati on-demand a runtime invece che prerenderati. `npm run build` completa con successo: le 4 route sitemap `/cards/s/{game}/sitemap/[__metadata_id__]` sono ora `ƒ` (dynamic) invece di prerenderate. Sessione precedente: fix filtri "Tipo carta" per Magic in `app/cards/[game]/CarteClient.tsx` — sostituiti i chip (prima autocomplete fetchato dal DB) con 8 macrotipi hardcodati (`MAGIC_CARD_TYPES`: Creatura/Instant/Stregoneria/Incantesimo/Artefatto/Terra/Planeswalker/Battaglia), tutti con `match: 'ilike'` cosicché la query Supabase esistente filtri con `card_type ilike '%value%'` e catturi anche le type line composite (es. "Artifact Creature — Golem"). Nessuna modifica al DB/API. Verificato in preview: i chip compaiono correttamente su `/cards/magic` e il click su "Creatura" genera la query `card_type=ilike.%25Creature%25`. **Nota**: ho riscontrato un bug preesistente e non collegato — `/cards/magic` ritorna 500 (`statement timeout`, codice Postgres `57014`) anche senza alcun filtro attivo, sia sulla query base sia su quella filtrata; `/cards/pokemon` invece funziona. Probabile indice mancante o dataset Magic troppo grande per `order by name, id` senza indice dedicato — da investigare in una sessione separata (richiede accesso DB/migrazioni, fuori scope per questo task). `npm run typecheck` e `npm run lint` puliti. Sessione precedente: fix 3 errori lint preesistenti — `app/(app)/hunts/actions.ts` (var `user` non usata → `_user`), `lib/validation/offer.ts` (funzione `shippingToCents` non usata → `_shippingToCents`), e plugin `eslint-plugin-react-hooks` mancante (installato e registrato in `eslint.config.mjs` con la regola `react-hooks/exhaustive-deps`). Risolvendo quest'ultimo è emerso che il commento `eslint-disable-line react-hooks/exhaustive-deps` in `components/auth/OnboardingWizard.tsx` (handleDrop) era diventato un disable-directive inutilizzato (la regola, ora attiva, non segnala nulla su quella riga) → rimosso. `npm run lint` ora esce con 0 errori e 0 warning. Sessione precedente: verifica riferimenti landing — `components/landing/LandingClient.tsx` non conteneva alcuna occorrenza di `/feed` o `/carte` (già pulito, usa solo ancore interne e `/signup`/`/privacy`). Unico file in `components/landing/` è `LandingClient.tsx`. I redirect `/feed`→`/market` e `/carte`→`/cards` in `next.config.ts` sono intenzionali (301 per i vecchi URL) e sono stati lasciati invariati. Sessione precedente: riscritti i filtri del catalogo (`/cards/[game]`) — chip hardcodati + autocomplete, rimosso il fetch dei valori distinti al mount e il filtro Set (vedi sezione "Filtri catalogo riscritti"). Sessione precedente: rinominate le route `/carte` → `/cards` e `/feed` → `/market` (cartelle, link, sitemap, redirect 301). Nei documenti sotto, i riferimenti storici a `/carte` e `/feed` vanno letti come `/cards` e `/market` (i file vivono ora in `app/cards/` e `app/(app)/market/`).
